<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST")
    {
        ...
        $sql = "INSERT INTO foptransaction (fid, dod, cost, name, city, mobile, email, pincode, addr)
                VALUES ('$fid', '$date', '$cost', '$name', '$city', '$mobile', '$email', '$pincode', '$addr')";
        ...
        $resu =mysqli_query($conn, "SELECT  MAX(tid) as max , MIN(tid) as min FROM foptransaction");
        ...
        $sqli = "INSERT INTO foporders (fid, pid, dod, tid)
                     SELECT fid, pid, datee, $max FROM fmycart WHERE fid = '$fid'";
        $sqlii = "DELETE FROM fmycart WHERE fid = '$fid'";
    ...
    <body>
        <div class="container">
            <div class="card">
                ...
                <form method="post" action="fonlinepay.php?pid=<?= $pid; ?>">
                <?php
            ...
            <div class="buttonrow">
                <a href="fcarddetailsok.php?pid=<?= $pid; ?>" class="but1"><span class="glyphicon glyphicon-shopping-cart">Back</a>
            </div>
        </div>
    </body>
</html>